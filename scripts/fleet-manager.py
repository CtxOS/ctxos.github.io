        ...
        <head>
            ...
            <script>
                setInterval(async () => {
                    ...
                    const mirrorSynced = data.mirror_synced;
                    const statusClass = mirrorSynced ? 'healthy' : 'warning';
                    const statusText = mirrorSynced ? 'SYNCED' : 'OUTDATED';
                    document.getElementById('mirror-status').className = 'badge ' + statusClass;
                    document.getElementById('mirror-status').textContent = statusText;
                }, 5000);
            </script>
        </head>
        <body>
